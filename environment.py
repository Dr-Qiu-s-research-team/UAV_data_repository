...
import numpy as np
import random
from generator import Obstacle_Generator
...
class Env(object):
    def __init__(self, args, obs_list, source, goal):
        self.mode = args.mode
        ...
        self.objs_info = {
            "obst_list": tuple(obs_list),
            "goal": goal.astype(np.float32),
            "drone_pos": source.astype(np.float32),
            "drone_pos_start": source.astype(np.float32)
        }

    def reset(self, env=True, source=None, destination=None, obs_list=None):
        '''Reset environment including:
            obstacle positions, goal position and drone position
        '''
        self.objs_info = self._reset_objs(env, source, destination, obs_list)
    ...
    def _calculate_projection_reward(self, drone_pos_curt, drone_pos_next):
        ...
        if projection_normalized < -1:
            projection_normalized = -1
        if projection_normalized > 1:
            projection_normalized = 1
    ...
    def _reset_objs(self, env, source, destination, obs_list):
        if env and obs_list is None:
            obst_list = []
...
if __name__ == '__main__':
    args = Args()
    generate = Obstacle_Generator(0,20)
    source = np.array([0,0,0])
    goal = np.array([5,5,5])
    env=Env(args,generate.obs_list, source, goal)
    print(env.get_state())
